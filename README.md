...
- **Capture `/in/{token}` is session-free** (no cookies, not on the `web` stack). nginx `client_max_body_size 1m` returns JSON 413; PHP also caps. Per-token throttle is 120/min, global 600/min.
